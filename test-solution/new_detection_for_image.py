...
def postprocess_patches(original_image, predictions, coordinates, threshold=0.1, iou_threshold=0.2):
    all_predictions = []
    results = []
...
def detect_from_video(video_surce, output_csv=None, output_images_folder='container_output'):

    while True:

        ret, image = cv2.VideoCapture(video_surce).read()
